...
public class WalletService {
    ...
    /** Internal account format: ACC- followed by 8 uppercase hex digits (generated in AuthService). */
    ...
    private void validateAmount(BigDecimal amount) {
        ...
        // stripTrailingZeros: 5.000 → 5 (valid), 5.001 → 5.001 (invalid – too many decimal places)
    ...
    /**
     * Loads the wallet balance with a pessimistic write lock to prevent race conditions
     * on concurrent withdrawals (double-spend protection).
     */
    ...
    private void handleExternalWithdrawal(Transaction transaction, WalletBalance sourceBalance) {
        updateBalance(sourceBalance, transaction.getAmount().negate());
        if (demoMode) {
            // Demo mode: instant completion without a real payment processor
            transaction.setStatus(Transaction.Status.COMPLETED);
        } else {
            // Production: balance deducted, waiting for payment gateway confirmation
            transaction.setStatus(Transaction.Status.PENDING);
        }
    }
    ...
    private void handleDeposit(Transaction transaction) {
        if (demoMode) {
            // Demo mode: instant credit without an external processor
            ...
            // Production: balance credited only after payment gateway confirms receipt
            transaction.setStatus(Transaction.Status.PENDING);
        }
    }
